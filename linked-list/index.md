...
- stack-by-ll-1.c  : The first implementation where the head is pointing to the address of the 1st node, and all the other nodes are like they stores the address of the forward attached node, for push we can store the adderess in a temp variable of the latest node but for the pop we will have to traverse the linked list to get the latest node pointer inefficient at poping
